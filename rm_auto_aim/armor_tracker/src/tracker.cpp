...
void Tracker::update(const Armors::SharedPtr & armors_msg)
{
  ...
  if (!armors_msg->armors.empty()) {
    ...
    // RCLCPP_WARN(rclcpp::get_logger("armor_tracker"), "min_position_diff: %f", min_position_diff);
    // RCLCPP_WARN(rclcpp::get_logger("armor_tracker"), "yaw_diff: %f", yaw_diff);
    // RCLCPP_WARN(rclcpp::get_logger("armor_tracker"), "max_match_yaw_diff_: %f", max_match_yaw_diff_);
    //RCLCPP_WARN(rclcpp::get_logger("armor_tracker"), "same_id_armors_count: %d", same_id_armors_count);
    ...
    if (min_position_diff < max_match_distance_ && yaw_diff < max_match_yaw_diff_) {
      ...
      //RCLCPP_WARN(rclcpp::get_logger("armor_tracker"), "tracking_: yaw_diff: %f", yaw_diff);
      ...
      RCLCPP_WARN(rclcpp::get_logger("armor_tracker"), "yaw_diff: %f", yaw_diff);
      RCLCPP_WARN(rclcpp::get_logger("armor_tracker"), "yaw_diff: %f",min_position_diff);
  ...
  if (tracker_state == DETECTING) {
    if (matched) {
      ...
      if (detect_count_ > tracking_thres) {
        ...
        //RCLCPP_WARN(rclcpp::get_logger("armor_tracker"), "tracker_state:TRACKING1" );
      }
    } else {
      detect_count_ = 0;
      tracker_state = LOST;
      //RCLCPP_WARN(rclcpp::get_logger("armor_tracker"), "tracker_state:LOST" );
    }
  } else if (tracker_state == TRACKING) {
    if (!matched) {
      tracker_state = TEMP_LOST;
      lost_count_++;
      //RCLCPP_WARN(rclcpp::get_logger("armor_tracker"), "tracker_state:TEMP_LOST" );
    }
    ...
    if (!matched) {
      ...
      tracker_state = TRACKING;
      lost_count_ = 0;
      //RCLCPP_WARN(rclcpp::get_logger("armor_tracker"), "tracker_state:TRACKING2" );
    }
  }


  //RCLCPP_WARN(rclcpp::get_logger("armor_tracker"), "tracker_state: %d",tracker_state);
...
void Tracker::handleArmorJump(const Armor & current_armor)
{
  ...
  if ((current_p - infer_p).norm() > max_match_distance_) {
    ...
    RCLCPP_WARN(rclcpp::get_logger("armor_tracker"), "current_p - infer_p %f ",(current_p - infer_p).norm() );
...
double Tracker::orientationToYaw(const geometry_msgs::msg::Quaternion & q)
{
  ...
  //RCLCPP_WARN(rclcpp::get_logger("armor_tracker"), "last_yaw_: %lf",last_yaw_ );
  last_yaw_ = yaw;
  return yaw;
}
